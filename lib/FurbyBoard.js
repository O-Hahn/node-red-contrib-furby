...
FurbyPiBoard.prototype.init = function() {
 ...
 FurbyPiBoard.prototype.SpeakOutput = function(msg, format, filename){
	...
	var myStreamBuffer = new streamBuffers.ReadableStreamBuffer({
	    frequency: 1,   // in milliseconds.
	    chunkSize: 1024  // in bytes.
	});	
	myStreamBuffer.put(msg.payload);
 	
    // send file to output
    myStreamBuffer.pipe(speaker);
};

FurbyPiBoard.prototype.FileSave = function(msg, format, filname) {
	if (filename === "") {
